...
GLuint Engine::Game::BuildShader(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
	...
	// Shader Program
	GLuint program_Player = glCreateProgram();
	glAttachShader(program_Player, vertex);
	glAttachShader(program_Player, fragment);
	if (geometryPath != nullptr)
		glAttachShader(program_Player, geometry);
	glLinkProgram(program_Player);
	CheckShaderErrors(program_Player, "PROGRAM");
	// Delete the shaders as they're linked into our program_Player now and no longer necessery
...
void Engine::Game::UseShader(GLuint program_Player)
{
	// Uses the current shader
	glUseProgram(program_Player);
}
